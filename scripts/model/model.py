    ...
    def __init__(self, file):
        ...
        with open(file, 'r') as f:
            ...
            for i in range(self.t):
                tri = list(map(int, f.readline().split()))[1:]
            ...
            for tri in self.triangles:
                self.edges.add(tuple(sorted((tri[0], tri[1]))))
                self.edges.add(tuple(sorted((tri[1], tri[2]))))
                self.edges.add(tuple(sorted((tri[2], tri[0]))))
