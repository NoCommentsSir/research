...
# Желательно грубо переориентировать нормали наружу
center = PC.points_arr.mean(axis=0)
for i in range(PC.points_arr.shape[0]):
    if np.dot(PC.points_arr[i] - center, PC.normals_arr[i]) < 0:
        PC.normals_arr[i] *= -1
...
x = tree.solve_hierarchical_poisson(
    normals=PC.normals_arr,
    max_depth=D,
    qq=Q,
    start_depth=3,
    radius=4
...
field, gamma = tree.get_dense_field(
    PC.points_arr,
    depth=D,
    x=x,
    q=96
...
# PC = generate_point_cloud(M,6000)
# root = OctreeNode(np.array([0.0, 0.0, 0.0]), 2.2, 0)
# tree = Octree(root, PC.points_arr, max_depth=D, min_points=1)
# tree.build()
...
# for depth in tqdm(range(3, D + 1)):
#     print(f"Solving depth {depth}")

#     tree.fill_depth_neighborhood(depth)

#     for node in tree.depth_map[depth].values():
#         node.vector_field[:] = 0.0

#     tree.calculate_normals(tree.points, PC.normals_arr, depth=depth)
#     tree.assign_node_ids(depth)

#     L = tree.assemble_L(qq=Q, depth=depth)
#     v = tree.calculate_v(qq=Q, depth=depth)

#     if x_prev is None:
#         x = lsmr(L, v)[0]
#     else:
#         x0 = tree.prolongate_solution(
#             x_prev,
...
# field, gamma = tree.get_dense_field(PC.points_arr, depth=D, x=x, q=64)
